...
class Complexify @JvmOverloads constructor(
    ...
    companion object {
        private val CHARSETS_ARRAY = arrayOf(
            // Commonly Used
            ////////////////////
    ...
    fun checkComplexityOfPassword(password: String, listener: ComplexityListener) {
        ...
        if (!shouldUseBanList || !isInBanList(password)) {
            ...
            for (i in CHARSETS_ARRAY.indices.reversed()) {
                complexity += additionalComplexityForString(password, CHARSETS_ARRAY[i]).toDouble()
            }
        } else {
            complexity = 1.0
        }
        ...
        complexity = ln(Math.pow(complexity, password.length.toDouble())) * (1.0 / strengthScaleFactor)
        val valid = complexity > MIN_COMPLEXITY && password.length >= minimumChars

        // Scale to percentage, so it can be used for a progress bar
        complexity = complexity / MAX_COMPLEXITY * 100
        complexity = if (complexity > 100) 100.0 else complexity
    ...
    private fun isInBanList(password: String): Boolean {
        val passwordLowerCase = password.toLowerCase(Locale.ROOT)
        return if (banMode === ComplexifyBanMode.STRICT) {
            ...
            for (s in banList) {
                if (s == passwordLowerCase) { //banList elements already in lower case
                    return true
                }
            }
            false
